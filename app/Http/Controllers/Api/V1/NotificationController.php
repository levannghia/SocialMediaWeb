...
class NotificationController extends Controller
{
    ...
    private function getAccessToken()
    {
        ...
        // Tạo ServiceAccountCredentials từ thông tin trong file JSON
        ...
        // Tạo HTTP client và thêm middleware để xác thực
        $httpHandler = Middleware::mapRequest(function ($request) use ($credentials) {
            return $credentials->applyToRequest($request);
        });
        $client = new Client(['handler' => $httpHandler]);

        // Tạo middleware để lấy token truy cập
        $middleware = new AuthTokenMiddleware($credentials);
        $client = $middleware->attach($client);

        // Gửi yêu cầu để lấy token truy cập
        $response = $client->request('GET', 'https://www.googleapis.com/oauth2/v4/token');

        // Giải mã phản hồi và trả về token truy cập
        $tokenResponse = json_decode((string) $response->getBody(), true);
        return $tokenResponse['access_token'];
    }

    // Hàm để tạo JWT token từ Service Account Credentials
    // private function jwtToken($key)
    // {
    ...
    //         'exp' => 2147483647, // Thời gian hết hạn token, ở đây là 1 giờ (3600 giây)
    //         'iat' => $now,
    //     ]));
